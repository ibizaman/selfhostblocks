...
The [`flake.nix`](./flake.nix) file sets up a Immich server with Self Host Blocks. There are actually 3 demos:

- The `basic` demo sets up a lone Immich server accessible through https.
- The `ldap` demo builds on top of the `basic` demo integrating Immich with a LDAP provider.
- The `sso` demo builds on top of the `ldap` demo integrating Immich with an SSO provider, backed by the LDAP provider.
...
## Deploy to the VM {#demo-immich-deploy}
...
### Deploy with nixos-rebuild {#demo-immich-deploy-nixosrebuild}
...
uri like `github:ibizaman/selfhostblocks?path=demo/immich#basic`

It is very important to remove leftover `nixos.qcow2` files, if any, as done in the snippet above.

### Deploy with Colmena {#demo-immich-deploy-colmena}
...
rm nixos.qcow2; \
  ...
  QEMU_NET_OPTS="hostfwd=tcp::2222-:2222,hostfwd=tcp::8080-:80" ./result/bin/run-nixos-vm -m 4096
```
...
neither LLDAP nor Immich will start.

## Immich Demos {#demo-immich-access}
...
### Basic demo {#demo-immich-basic}

Assuming you deployed the `basic` demo,
go to [https://i.example.com](https://i.example.com) and you will be greeted with the 
Immich login screen.

You can login with the declaratively provisioned admin user:

- Email: `admin@example.com`
- Password: `immichadmin`

::: {.info}
If you're too fast, you might get some nginx bad gateway errors or even the Immich
web UI showing up telling you you need to pick the server. This is because Immich takes a good
minute to start and until then, you won't see the login screen. Either wait a bit or in the
terminal where you run the SOCKS proxy, tail the logs with `journaltcl -f -u immich` and wait
for the message `Main: Startup complete`.
:::

And that's the end of the demo.

### LDAP demo {#demo-immich-ldap}

After Immich has started (see previous section's note),
go to [https://i.example.com](https://i.example.com) and you will be greeted with the 
Immich login screen.
...
### SSO demo {#demo-immich-sso}

After Immich has started (wait up to 2 minutes, see previous section's note),
go to [https://i.example.com](https://i.example.com) and you will be greeted with the 
Immich login screen.
...
See [the manual](https://shb.skarabox.com/services-immich.html#services-immich-sso) for more information.
:::
...
## In More Details {#demo-immich-in-more-details}

### Files {#demo-immich-files}
...
- [`flake.nix`](./flake.nix): nix entry point, defines one target host for
  ...
  setting up the Immich server paired with the LDAP and SSO server.
...
- Secrets related files:
  ...
  - [`secrets.yaml`](./secrets.yaml): encrypted file containing required secrets for Immich
...
### Virtual Machine {#demo-immich-virtual-machine}
...
#### Why do we need the VM's public key {#demo-immich-tips-public-key-necessity}
...
### Deploy {#demo-immich-tips-deploy}
...
### Update Demo {#demo-immich-tips-update-demo}
