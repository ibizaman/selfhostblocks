...
in
{
  ...
  backup = shb.runNixOSTest {
    ...
    nodes.server =
      { config, ... }:
      {
        imports = [
          basic
          (shb.backup config.shb.nextcloud.backup)
        ];
      };
  ...
  previewGenerator = shb.runNixOSTest {
  ...
  externalStorage = shb.runNixOSTest {
