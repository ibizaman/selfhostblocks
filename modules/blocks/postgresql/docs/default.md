...
PostgreSQL requires an explicit major version:

```nix
shb.postgresql.version = 17;
```
...
an evaluation error. Setting the option does not upgrade the database.
...
Changing this option does not upgrade a database. If SHB finds a cluster for a
different PostgreSQL major, it refuses to initialize a new cluster and reports
the upgrade helper to run.
...
3. Stop services that use PostgreSQL, then stop `postgresql.service`.
4. Run `sudo upgrade-pg-cluster-15-18`.
5. Set `shb.postgresql.version = 18;` and deploy.
6. Validate all database consumers and take a fresh backup before manually
